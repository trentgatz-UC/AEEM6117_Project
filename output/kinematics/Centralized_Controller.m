...
save_dir = 'C:\Users\trent\OneDrive - University of Cincinnati\2024_S2\AEEM6117_Project\output\kinematics';
% %% Givens
...
% 
% %% Setting up FIS
% fis = readfis("kinematics_mediumGA_pop350");
...
%fis_options.MethodOptions.PopulationSize = 600;
...
%writeFIS(fis_trained, fullfile(save_dir, "Kinematics_HeavyGA_pop600"))
%save(fullfile(save_dir, "Kinematics_HeavyGA_pop600"))

%% rerun & make video
[tout, yout] = sim_system(tspan, target, robots, k, m, l0, fis_trained);

vid_framerate = 24; % video frame rate (frames / second)
vid_name = fullfile(save_dir,'Kinematics_HeavyGA_pop600');
figure
...
%}
d2ydt2 = zeros(10,1);
d2ydt2(1) = (1/3)*(rbi_hat(1,1)*(p(1)))+(1/3)*(rbi_hat(2,1)*(p(2)))+(1/3)*(rbi_hat(3,1)*(p(3)));
d2ydt2(2) = 0; % should this last term be divided by m ?, make system stiffer
d2ydt2(3) = (1/3)*(rbi_hat(1,2)*(p(1)))+(1/3)*(rbi_hat(2,2)*(p(2)))+(1/3)*(rbi_hat(3,2)*(p(3)));
d2ydt2(4) = 0;
